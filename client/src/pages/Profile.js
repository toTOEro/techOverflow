...
const Profile = () => {
  ...
  const { data: regiData } = useQuery(REGISTERED, {
  ...
  if (!Auth.loggedIn()) {
    return (
      <Heading
        bgGradient="linear(to-r, #D21D26, #C8102E)"
        bgClip="text"
        py="40vh"
        fontSize="7xl"
        textAlign="center"
        ...
        <Stack>
          ...
          <Grid templateColumns={"repeat(2, 1fr)"}>
            <GridItem>
              ...
              <List>
                {registered.length > 0 ? (
                  registered.map(({ _id, title }) => (
                    <ul key={_id}>
                      <ReactLink to={`../posting/${_id}`} target={"_blank"}>
                        {title}
                      </ReactLink>
                    </ul>
                  ))
            ...
            <GridItem>
              ...
              {postings.length > 0 ? (
                postings.map(({ _id, title, description }) => (
                  <ul key={_id}>
                    <ReactLink to={`../posting/${_id}`} target={"_blank"}>
                      {title}
                    </ReactLink>
